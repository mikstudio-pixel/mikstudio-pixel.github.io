document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDynamicTint() {
        if (!isHomePage) {
            return;
        }

        const activeCard = getActiveBackCard();
        if (!activeCard) {
            return;
        }

        applyDynamicTint(getCardTint(activeCard));
    }
    ...
    function scheduleDynamicTintUpdate() {
        ...
        if (tintState.updateTimer !== null) {
            window.clearTimeout(tintState.updateTimer);
        }

        tintState.updateTimer = window.setTimeout(() => {
            tintState.updateTimer = null;
            updateDynamicTint();
        }, 80);
    }
    ...
    function initDynamicTint() {
        ...
        backContent.querySelectorAll('img').forEach((img) => {
            if (img.complete && img.naturalWidth > 0) {
                return;
            }

            img.addEventListener('load', scheduleDynamicTintUpdate, { once: true });
            img.addEventListener('error', scheduleDynamicTintUpdate, { once: true });
        });
